    ...
    public class DSRequest
    {
        ...
        public void SetRecord(string attribute, JToken value)
        {
            ...
            if (RequestIndex >= 0)
            {
                ...
                if ( Action == "Update")
                    record = record["New"] as JObject;

                if (!(record[attribute] is JArray values))
                {
                    values = new JArray();
        ...
        public void UpdateRecord(JObject newRecord)
        {
            ...
            JObject target = ( Action == "Update" ? Request["record"]["New"] : Request["record"]) as JObject;
            ...
            else
            {
                ...
                foreach (JProperty property in newRecord.Properties())
                {
                    if (!(target[property.Name] is JArray values))
                    {
                        values = new JArray();
