...
| `duration` | le masque — **la virgule : l'heure ou la minute en centièmes, l'heure en dix-millièmes** ; les unités `s`/`min`/`h`/`d`/`w`/`m`/`y` (D476) ; **les sous-items au point** — `.days`, `.months`… (les fonctions du type — D772–D773/D838) ; **le constructeur `duration(valeur, unité)`** (D659/D975 — `duration(NOCNTPSOUV, h)`) ; **les conversions du type, à paramètres** (D975 — la base d'un temps industriel en centièmes vers les heures-minutes) ; **la matrice des temps** (D976/D978) — la standard fournie par le type (s/min/h/d/w/m/y), **la matrice propre au champ**, **les unités supplémentaires apportées par les données avec leur coefficient vers une unité connue** : `F` la frappe de tel poste, « 20 F/H ⇒ 3 min » (*en proposition : `duration(v, F, { F: { h: 1 / 20 } })`*) ; la transitivité et le chemin obligatoire vers une unité standard (D979 — « cette approche fonctionne également pour les durées ») | le nul = 0 | D380, D476, D838, D975–D979 |
