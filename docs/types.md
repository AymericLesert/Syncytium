...
| `barcode` | **la nature du code en paramètre** (D987 — « barcode me convient avec un type de code-barres en paramètre ») : `barcode[ean]` à la déclaration (*la nature au crochet, mienne*) — ean (les 13 chiffres et la clé de contrôle), ean8, upc, code128, qr… (*la liste mienne*) ; **la validation intégrée selon la nature** ; **le constructeur `barcode(<nature>, valeur)`** — `barcode(ean, ARCTEAN13)` : la valeur validée à l'exécution, sans conflit de longueur statique (D581) ; une valeur présente et fausse = non conforme (D933) ; la sortie code-barres (D300 — les composants `barcode`/`qrcode`) s'appuie sur le type | D300, D391, D987 |
