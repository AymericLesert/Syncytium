...
namespace Syncytium.Core.Common.Server.Database.DSSchema
{
    ...
    public class DSDatabase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public override string ToString() => ToJSON(String.Empty, UserProfile.EUserProfile.None, null).ToString(Formatting.None);
        ...
        /// <returns></returns>
        public static string GetDirectory(Type enumeration) => (GetArea(enumeration.Namespace) ?? String.Empty) + "/" + enumeration.Name + "/";
        ...
        public DSDatabase(Type schema, IDSRequest request)
        {
            ...
            if (schema.IsSubclassOf(typeof(DbContext)))
            {
                ...
                foreach (PropertyInfo property in schema.GetProperties())
                {
                    ...
                    DSTable table = new(this, property, property.PropertyType.GetGenericArguments().First());
