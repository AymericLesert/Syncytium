...
namespace Syncytium.Core.Common.Server.Managers
{
    ...
    public sealed class LoggerManager : ILogger, IDisposable
    {
        ...
        /// </summary>
        private readonly Stack<TextWriter> _logFiles = new();
        ...
        public void Initialize()
        {
            ...
            try
            {
                ...
                WriteMessage(Level.Info, MODULE_NAME, "Version    : " + ConfigurationManager.ApplicationVersion?.ToString() ?? "Undefined");
        ...
        private void CleanUpLogFiles(bool force = false)
        {
            ...
            Regex pattern = new(@"^Syncytium.*\-(?<year>\d+)\-(?<month>\d+)\-(?<day>\d+)(|\.\d+)\.log$");
            ...
            foreach (string filenameAndPath in Directory.GetFiles(Path.GetDirectoryName(currentLogFilename) ?? String.Empty))
            {
                ...
                try
                {
                    DateTime logDate = new(int.Parse(match.Groups["year"].Value),
        ...
        public void Exception(string module, string message, System.Exception exception)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (newLine.Length > 0 && newLine[^1] < 32)
