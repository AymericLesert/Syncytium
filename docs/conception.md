...
| D94 | **Dépréciation par version minimale supportée (résout Q8)** : la description déclare une **version minimale supportée** (pas une durée) ; un appel API sous ce seuil reçoit **419**. | Seuil de version explicite, rythme au technicien (cf. D12). Réserve : 419 non standard — alternatives 426 Upgrade Required / 410 Gone, à confirmer. |
| D95 | **« Technicien » = un rôle (résout Q14)** : le **responsable technique de la solution déployée** (l'éditeur, un intégrateur, ou un adopteur compétent), distinct des utilisateurs finaux. | Terme générique, pas une personne. |
| D96 | **Résurrection des affectations (résout Q25)** : si un groupe supprimé **réapparaît** (clé = **identifiant stable**, pas le libellé), les affectations conservées **reprennent vie**. | Résilience aux suppressions par inadvertance (ajustements de sécurité SI). Cohérent avec D34 (affectations conservées, non purgées ; notification). |
...
| ~~Q8~~ | ~~Fenêtre de support / durée de dépréciation ?~~ | **Résolu (D12, D94)** : pas une durée mais une **version minimale supportée** déclarée ; appel sous le seuil → **419** (code à confirmer, cf. D94). |
...
| ~~Q29~~ | ~~Calibration du modèle de risque ?~~ | **Architecture résolue (D47, D50, D51)**. Reste, en **calage empirique** (réglage, pas architecture) : (1) taille de la fenêtre glissante ; (2) seuil en échelle **log** (le défaut D51 ne couvre que le linéaire) ; (3) seuil du **détecteur de pics** (z-score ?) ; (4) seuil d'**étendue/couverture** (crawl) ; (5) garde-fou **R²**. À calibrer sur données réelles. |
...
| ~~Q14~~ | ~~Modèle de déploiement / qui est le technicien ?~~ | **Résolu (D16, D17, D95)** : une instance par TPE, moteur public, mise à jour technique manuelle, description à chaud (§7.2). Le **« technicien » = un rôle** (responsable technique : éditeur / intégrateur / adopteur compétent), distinct des utilisateurs (D95). |
