...
namespace Syncytium.Common.Database.Provider.Firebird
{
    /// <summary>
    /// Handle the access to the firebird database
    /// </summary>
    public class Firebird : Provider
    {
        #region Logger
        ...
        /// </summary>
        protected override string MODULE => typeof(Firebird).Name;
        ...
        /// </summary>
        public override EProvider Type => EProvider.Firebird;
        ...
        /// <returns></returns>
        public override string GetSQLExistTable(string table) => $"SELECT RDB$RELATION_NAME FROM RDB$RELATION_FIELDS WHERE RDB$RELATION_NAME = '{table}'";
        ...
        public override bool ExecuteScript(string script)
        {
            ...
            {
                if (command.ToUpper().StartsWith("SET TERM ") || String.IsNullOrWhiteSpace(command))
                    continue;
        ...
        public Firebird(DbConnection database, string schema) : base(database, schema) { }
    }
}