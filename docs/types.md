...
| `duration` | le masque — **la virgule : l'heure ou la minute en centièmes, l'heure en dix-millièmes** ; les unités `s`/`min`/`h`/`d`/`w`/`m`/`y` (D476) ; **les sous-items au point** — `.days`, `.months`… (les fonctions du type — D772–D773/D838) ; **le constructeur `duration(valeur, unité)`** (D659/D975 — `duration(NOCNTPSOUV, h)`) ; **les conversions du type, à paramètres** (D975 — la base d'un temps industriel en centièmes vers les heures-minutes) ; **la matrice des temps** (D976) — la standard fournie (s/min/h/d/w/m/y), **les unités étendues** : `F` la frappe, « 20 F/H ⇒ 3 min » — le coefficient à l'intersection (`duration: { F: { h: 1 / 20 } }`, *en proposition*) | le nul = 0 | D380, D476, D838, D975–D976 |
...
| `measure` | les unités : **statiques** (`units: [kg, g, t]`), **la table de référence** (`units: stock.unit`), ou **libres** (défaut) ; **le constructeur `measure(valeur, unité)`** (D659/D975 — `measure(ARCNPDSUNI, kg)`, l'unité parmi celles du champ, hors liste = une erreur d'ingestion D581) ; **les conversions du type, à paramètres** (D975 — « des règles de conversion qui peuvent nécessiter des paramètres complémentaires » : la masse volumique d'un volume vers un poids) — *la forme `.to(<unité>, <paramètres>)` en proposition* ; **les matrices de conversion** (D976) : les unités en lignes et en colonnes, le coefficient à l'intersection — **les matrices standard fournies** (kg/g/t, mm/cm/m…), **les unités étendues** par le technicien (`PL` la plaque), **le coefficient constant ou expression** quand il dépend du contexte (la plaque vers la masse : les dimensions, l'épaisseur, la densité de la matière) — *la déclaration en proposition : `conversions:` aux settings, en cascade (D359), `measure: { PL: { kg: <expression> } }`* ; **la matrice portée par l'enregistrement** (D977 — l'autre approche : le coefficient stocké, `article.conversions: list of [source, cible]` avec sa cellule `coefficient`, les deux couples de PMI), *consultée avant celles des settings — mien* | D391, D975–D977 |
