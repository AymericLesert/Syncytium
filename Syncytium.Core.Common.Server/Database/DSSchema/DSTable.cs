...
namespace Syncytium.Core.Common.Server.Database.DSSchema
{
    ...
    public class DSTable
    {
        ...
        /// </summary>
        public Type? SourceTable { get; } = null;

        /// <summary>
        /// Class containing the table description in case of a history table
        /// </summary>
        public String SourceTableName { get; } = String.Empty;
        ...
        private string GetSequenceKey(DatabaseContext database, DSColumn column, int id)
        {
            ...
            return (column.Property.GetValue(currentRecord) as string) ?? String.Empty;
        }
        ...
        private Tuple<DSRecord, InformationRecord?> GetRecordById(DatabaseContext database, int id)
        {
            ...
            if (id < 0)
                currentRecord = Table.GetConstructor(Array.Empty<Type>())?.Invoke(Array.Empty<object>()) as DSRecord;
            else
                currentRecord = database.Set(Table).Find(id) as DSRecord;
        ...
        private DSRecord GetRecordFromClient(DatabaseContext database, int customerId, int userId, string area, UserProfile.EUserProfile profile, string action, JObject record, JObject identity, bool check)
        {
            ...
            if (record[Columns[_indexKey].Property.Name] != null && record[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                currentId = record[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? 0;
            ...
            if (currentId < 0  && identity[Columns[_indexKey].Property.Name] != null && identity[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer && action != "Create")
                currentId = DatabaseCacheManager.Instance.GetId(database._Information, identity[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? 0, userId, this.Name);
            ...
            foreach (DSColumn column in Columns)
            {
                ...
                JToken? token = record[column.Property.Name];

                if (token == null)
                    continue;

                if (token.Type == JTokenType.Null && column.Sequence != null)
                {
                    ...
                    {
                        int currentSourceId = record["HistoryId"]?.ToObject<int>() ?? -1;
                        if (currentSourceId < 0)
                            currentSourceId = DatabaseCacheManager.Instance.GetId(database._Information, identity["HistoryId"]?.ToObject<int>() ?? -1, userId, SourceTableName);
                        value = GetSequenceKey(database, column, currentSourceId);
                    }
                    else value = action switch
                    {
                        "Create" => database.GetSequenceKey(customerId, column.Sequence.Key, column.Sequence.Length),
                        _ => GetSequenceKey(database, column, currentId),
                    };
                }
                else
                {
            ...
            if (record[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                Columns[_indexKey].Property.SetValue(currentRecord, currentId);

            foreach (DSColumn column in Columns)
            {
                DSForeignKeyAttribute? foreignKey = column.ForeignKey;
                object? foreignKeyIdValue = column.Property.GetValue(currentRecord);

                if (foreignKey == null || foreignKeyIdValue == null)
                    continue;

                // Get the id of the foreign key

                int foreignKeyId = (int)foreignKeyIdValue;
                if (foreignKeyId >= 0)
                    continue;
                ...
                int id = DatabaseCacheManager.Instance.GetId(database._Information, identity[column.Property.Name]?.ToObject<int>() ?? -1, userId, foreignKey.Table);
                if (id < 0)
                {
                    errors.AddField(column.Property.Name, foreignKey.Error, new object[] { $"{{{column.Field}}}" });
                    continue;
            ...
            if (check)
            {
                foreach (DSColumn column in Columns)
                {
                    ...
                    if (fields != null)
                    {
                        ...
                        {
                            fieldsStr += (fieldsStr == string.Empty ? "" : ",") + string.Format("{0}={1}", v.Key, v.Value?.ToString() ?? String.Empty);
                        }
                    }

                    Warn($"Key ({Name}.{column.ColumnName} = '{currentValue}' / {fieldsStr}) is not unique ! CustomerId: {customerId}, UserId: {userId}, Area: {area}, Profile: {profile}, Action: {action}, Record: {record.ToString(Formatting.None)}, Identity: {identity.ToString(Formatting.None)}");
            ...
            if (record["_tick"] != null && record["_tick"]?.Type == JTokenType.Integer)
                currentRecord._tick = record["_tick"]?.ToObject<int>() ?? -1;

            if (record["_deleted"] != null && record["_deleted"]?.Type == JTokenType.Boolean)
                currentRecord._deleted = record["_deleted"]?.ToObject<bool>() ?? false;
        ...
        {
            List<Tuple<DSRecord, InformationRecord?>> result = new();

            // Build the list of new records

            List<DSRecord> records = new();
            ...
            List<InformationRecord> informations = new();
            ...
            foreach (DSRequest request in lot)
            {
                ...
                InformationRecord information = new()
                {
                    Table = Name,
                    Id = currentRecord.Id,
                ...
                result.Add(Tuple.Create<DSRecord, InformationRecord?>(currentRecord, information));
        ...
        private List<Tuple<DSRecord, InformationRecord?>> UpdateRecord(DatabaseContext database, int customerId, int userId, string area, UserProfile.EUserProfile profile, List<DSRequest> lot)
        {
            ...
            foreach (DSRequest request in lot)
            {
                ...
                if (record["Old"] is not JObject oldRecord || record["New"] is not JObject newRecord || identity["Old"] is not JObject oldIdentity || identity["New"] is not JObject newIdentity)
                {
                    Error($"[{Name}] : On updating a record, the reference on Old and New record are missing!");
                    throw new ExceptionDefinitionRecord("ERR_REQUEST_UPDATE_MISSING");
                }
                ...
                Tuple<DSRecord, InformationRecord?> existingRecord = GetRecordById(database, (int)(Columns[_indexKey].Property.GetValue(previousRecord) ?? -1));
                ...
                foreach (PropertyInfo property in Table.GetProperties())
                {
                    ...
                    else
                    {
                        ...
                        if (existingValue != null && nextValue != null && property.PropertyType == typeof(byte[]) && !((byte[])existingValue).SequenceEqual((byte[])nextValue))
                        {
                            if (IsVerbose())
                                Verbose($"[{Name}] : The previous value of property '{property.Name}' ({existingValue}) is replaced by '{nextValue}'");
                            property.SetValue(existingRecord.Item1, nextValue);
                        ...
                        if (existingValue != null && nextValue != null && property.PropertyType != typeof(byte[]) && !existingValue.Equals(nextValue))
                        {
                            if (IsVerbose())
                                Verbose($"[{Name}] : The previous value of property '{property.Name}' ({existingValue}) is replaced by '{nextValue}'");
                            property.SetValue(existingRecord.Item1, nextValue);
                ...
                if (existingRecord.Item2 == null)
                {
                    ...
                    existingRecord = Tuple.Create<DSRecord, InformationRecord?>(existingRecord.Item1, information);
                }
                else
                {
                    // update information attached to this record
        ...
        private List<Tuple<DSRecord, InformationRecord?>> DeleteRecord(DatabaseContext database, int customerId, int userId, string area, UserProfile.EUserProfile profile, List<DSRequest> lot)
        {
            ...
            foreach (DSRequest request in lot)
            {
                ...
                Tuple<DSRecord, InformationRecord?> existingRecord = GetRecordById(database, (int)(Columns[_indexKey].Property.GetValue(currentRecord) ?? -1));
        ...
        public JObject? ToJSON(string area, UserProfile.EUserProfile profile)
        {
            ...
            JArray jRecords = new();
            foreach (object? currentRecord in Columns.Select(column => column.ToJSON(area, profile)).Where(column => column != null).ToArray())
                if (currentRecord != null)
        ...
        public IEnumerable<Tuple<DSRecord, InformationRecord>> ReadRecords(DatabaseContext database)
        {
            ...
            List<Tuple<DSRecord, InformationRecord>> query = new(Capacity);

            if (records != null && database._Information != null)
            {
        ...
        {
            IEnumerable<DSRecord> records;
            if (Property.GetValue(database) is IEnumerable<DSRecordWithCustomerId> recordsWithCustomerId)
                records = recordsWithCustomerId.Where(r => r.CustomerId == customerId);
            else if (Property.GetValue(database) is IEnumerable<DSRecord> recordsTmp)
                records = recordsTmp;
            else
                throw new ExceptionParse("Unable to cast the attribute 'records'");

            List<Tuple<DSRecord, InformationRecord>> query = new(Capacity);

            if (database._Information != null)
            {
                query.AddRange(from record in records
                               join information in database._Information.Where(info => info.Table.Equals(Name)) on record.Id equals information.Id into gj
                               from subInformation in gj.DefaultIfEmpty()
        ...
        public Tuple<DSRecord, InformationRecord>? GetRecord(DatabaseContext database, int id, int customerId)
        {
            ...
            if (currentRecord.Item2 != null)
                return Tuple.Create(currentRecord.Item1, currentRecord.Item2);
        ...
        public int GetRecordId(DatabaseContext database, int userId, string action, JObject record, JObject identity)
        {
            ...
            switch (action)
            {
                ...
                case "Delete":
                    if (record[Columns[_indexKey].Property.Name] != null && record[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                        recordServerId = record[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? -1;

                    if (identity[Columns[_indexKey].Property.Name] != null && identity[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                        recordClientId = identity[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? -1;
                ...
                case "Update":
                    if (record["New"] != null && record["New"]?[Columns[_indexKey].Property.Name] != null && record["New"]?[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                        recordServerId = record["New"]?[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? -1;
                    ...
                    if (identity["New"] != null && identity["New"]?[Columns[_indexKey].Property.Name] != null && identity["New"]?[Columns[_indexKey].Property.Name]?.Type == JTokenType.Integer)
                        recordClientId = identity["New"]?[Columns[_indexKey].Property.Name]?.ToObject<int>() ?? -1;
        ...
        public Tuple<DSRecord?, DSRecord?>? ExtractRecord(DatabaseContext database, DSTransaction transaction, DSRequest request)
        {
            ...
            switch (request.Action)
            {
                ...
                case "Update":
                    ...
                    if (request.Record["Old"] is not JObject oldRecord || request.Record["New"] is not JObject newRecord ||
                        request.Identity["Old"] is not JObject oldIdentity || request.Identity["New"] is not JObject newIdentity)
                    {
                        Error($"[{Name}] : On updating a record, the reference on Old and New record are missing!");
                        throw new ExceptionDefinitionRecord("ERR_REQUEST_UPDATE_MISSING");
                    }
        ...
        public DSTable(DSDatabase databaseSchema, PropertyInfo propertyTable, Type table)
        {
            ...
            foreach (object annotation in table.GetCustomAttributes(true))
            {
                if (typeof(TableAttribute).IsInstanceOfType(annotation))
                    ...
                    Allow.Add((DSAllowAttribute)annotation);
                else if (typeof(DSLotAttribute).IsInstanceOfType(annotation))
                {
                    LotSize = ((DSLotAttribute)annotation).Size < 1 ? 1 : ((DSLotAttribute)annotation).Size;
                    Capacity = ((DSLotAttribute)annotation).Capacity < 1 ? 1 : ((DSLotAttribute)annotation).Capacity;
                }
            }

            // Create an instance of the record (table) to get default values for each properties

            object? instance = table.GetConstructor(Type.EmptyTypes)?.Invoke(Array.Empty<object>());
            if (instance != null)
            {
                ...
                foreach (PropertyInfo property in properties)
                {
                    ...
                    DSColumn column = new(databaseSchema, Name, property, instance);
                    Columns.Add(column);
                ...
                if (Name.StartsWith("History") && table.GetProperty("HistoryId") != null && databaseSchema.Schema.GetProperty(Name[7..]) != null)
                {
                    PropertyInfo? sourceTableProperty = databaseSchema.Schema.GetProperty(Name[7..]);
                        ...
                        sourceTableProperty.PropertyType.GetGenericArguments().First().IsSubclassOf(typeof(DSRecord)))
                    {
                        ...
                        if (SourceTable != null)
                        {
                            ...
                            {
                                if (typeof(TableAttribute).IsInstanceOfType(annotation))
                                    SourceTableName = ((TableAttribute)annotation).Name;
                            }
                        }
                    }
                }
