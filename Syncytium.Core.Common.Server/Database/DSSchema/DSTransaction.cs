...

{
    ...
    public class DSTransaction
    {
        ...
        /// </summary>
        public readonly JObject Label;

        /// <summary>
        /// List of all requests included into the transaction
        /// </summary>
        [JsonIgnore]
        public readonly List<DSRequest> Requests;

        /// <summary>
        /// List of requests grouped by lot
        /// </summary>
        [JsonIgnore]
        public readonly List<List<DSRequest>> LotRequests;

        /// <summary>
        /// List of compressed requests included into the transaction
        /// </summary>
        [JsonIgnore]
        public readonly JObject[] CompressedRequests;
            ...
            bool notify)
        {
            ...
            foreach (JObject request in requests)
            {
                ...
                if (table == null ||
                    action == null ||
                    request["identity"] is not JObject identity ||
                    request["record"] is not JObject record)
                {
                    LoggerManager.Instance.Error("DSTransaction", $"The request[{index}] isn't correctly formatted!");
                ...
                if (tick == null || tick.Type != JTokenType.Array)
                {
                    ...
                    DSRequest newRequest = new(index, table, action, record, identity, tickValue, request, -1);
                ...
                for (int i = 0; i < ticks.Count; i++)
                {
                    ...
                    DSRequest newRequest = new(index, table, action, transactionRecord, transactionIdentity, tickValue, request, i);
