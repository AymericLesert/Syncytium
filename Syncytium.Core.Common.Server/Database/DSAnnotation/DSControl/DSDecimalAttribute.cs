...
namespace Syncytium.Core.Common.Server.Database.DSAnnotation.DSControl
{
    ...
    public class DSDecimalAttribute : DSControlAttribute
    {
        ...
        /// <summary>
        /// Flag indicating if the value can be negative
        /// </summary>
        ...
        public override JObject ToJSON()
        {
            ...
            result["AllowNegativeValue"] = AllowNegativeValue;
        ...
        public override bool Check(DSColumn column, object? value, Errors errors)
        {
            ...
            if (validity)
            {
                ...
                if (valueRounded != valueToCheck ||
                    valueToCheck >= _maxValue || valueToCheck <= _minValue ||
                    (valueToCheck < 0 && !AllowNegativeValue))
                {
                    errors.AddField(column.Property.Name, Error, new[] { $"{{{column.Field}}}", $"{Digit}", $"{Precision}", $"{value}" });
                    return false;
                }
