...
namespace Syncytium.Core.Common.Server.Database
{
    ...
    public abstract class DatabaseContext : DbContext
    {
        ...
        /// Table "_Parameter"
        /// </summary>
        public DbSet<ParameterRecord>? _Parameter { get; set; } = null;

        /// <summary>
        /// Table "_Connection"
        /// </summary>
        public DbSet<ConnectionRecord>? _Connection { get; set; } = null;

        /// <summary>
        /// Table "_RequestId"
        /// </summary>
        public DbSet<RequestIdRecord>? _RequestId { get; set; } = null;

        /// <summary>
        /// Table "_Request"
        /// </summary>
        public DbSet<RequestRecord>? _Request { get; set; } = null;

        /// <summary>
        /// Table "_Request"
        /// </summary>
        public DbSet<RequestTableRecord>? _RequestTable { get; set; } = null;

        /// <summary>
        /// Table "_SequenceId"
        /// </summary>
        public DbSet<SequenceIdRecord>? _SequenceId { get; set; } = null;

        /// <summary>
        /// Table "_Information"
        /// </summary>
        public DbSet<InformationRecord>? _Information { get; set; } = null;

        /// <summary>
        /// Table "_Ping"
        /// </summary>
        public DbSet<PingRecord>? _Ping { get; set; } = null;
        ...
        /// </summary>
        /// <returns></returns>
        protected bool IsVerboseAll() => LoggerManager.Instance.IsVerboseAll;

        /// <summary>
        /// Indicates if the verbose mode is enabled or not
        /// </summary>
        /// <returns></returns>
        protected bool IsVerbose() => LoggerManager.Instance.IsVerbose;

        /// <summary>
        /// Log a verbose message
        /// </summary>
        /// <param name="message"></param>
        protected void Verbose(string message) => LoggerManager.Instance.Verbose(MODULE, message);

        /// <summary>
        /// Indicates if the debug mode is enabled or not
        /// </summary>
        /// <returns></returns>
        protected bool IsDebug() => LoggerManager.Instance.IsDebug;

        /// <summary>
        /// Log a debug message
        /// </summary>
        /// <param name="message"></param>
        protected void Debug(string message) => LoggerManager.Instance.Debug(MODULE, message);

        /// <summary>
        /// Log an info message
        /// </summary>
        /// <param name="message"></param>
        protected void Info(string message) => LoggerManager.Instance.Info(MODULE, message);

        /// <summary>
        /// Log a warn message
        /// </summary>
        /// <param name="message"></param>
        protected void Warn(string message) => LoggerManager.Instance.Warn(MODULE, message);

        /// <summary>
        /// Log an error message
        /// </summary>
        /// <param name="message"></param>
        protected void Error(string message) => LoggerManager.Instance.Error(MODULE, message);
        ...
        /// <param name="message"></param>
        /// <param name="ex"></param>
        protected void Exception(string message, System.Exception ex) => LoggerManager.Instance.Exception(MODULE, message, ex);
            ...
            foreach (PropertyInfo property in this.GetType().GetProperties())
            {
                ...
                foreach (PropertyInfo column in tableType.GetProperties())
                {
                    ...
                    foreach (object annotation in column.GetCustomAttributes(true))
                    {
                        ...
                        object? entityConfig = modelBuilder.GetType().GetMethod("Entity")?.MakeGenericMethod(tableType).Invoke(modelBuilder, null);
                        ...
                        try
                        {
                            DecimalPropertyConfiguration? decimalConfig = null;
                            ...
                            decimalConfig?.HasPrecision((byte)((DSDecimalAttribute)annotation).Digit, (byte)((DSDecimalAttribute)annotation).Precision);
                        }
                        catch (System.Exception ex)
                        {
                            if ( !firstError )
                            {
                                Info($"Unable to set decimal on the property {property.Name}.{column.Name} but it's not so important at all ({ex.Message}) !");
                                firstError = true;
                            }
                        }
                    }
        ...
        /// </summary>
        /// <param name="appDomainAppPath">Root directory of the application</param>
        /// <returns>true if the database is up to date</returns>
        public bool HasToUpgrade(string appDomainAppPath)
        {
            ...
            {
                LoggerManager.Instance.Warn(LoggerManager.MODULE_NAME, "The DS Model is not defined");
                return true;
            }

            LoggerManager.Instance.Info(LoggerManager.MODULE_NAME, "The DS Model is defined");

            // Retrieve the last upgrading already done

            int lastUpgradingVersion = 0;

            ParameterRecord? parameter = _Parameter?.SingleOrDefault(e => e.Key.Equals("Database.Version"));
            ...
            try
            {
                Debug($"The domain is '{appDomainAppPath}'");
            }
            ...
            Regex pattern = new(@"^(?<version>\d+)\-(?<year>\d+)\-(?<month>\d+)\-(?<day>\d+)-(?<label>.*)\.txt$");

            foreach (string sqlFilenameAndDirectory in Directory.GetFiles(Path.Combine(appDomainAppPath, "Database", "Provider", _provider.Type.ToString(), "Script")))
            {
        ...
        /// </summary>
        /// <param name="appDomainAppPath">Root directory of the application</param>
        public void Upgrade(string appDomainAppPath)
        {
            ...
            Regex pattern = new(@"^(?<version>\d+)\-(?<year>\d+)\-(?<month>\d+)\-(?<day>\d+)-(?<label>.*)\.txt$");
            Dictionary<int, string> scriptsToRun = new();
        ...
        public DatabaseLock Lock(int customerId)
        {
            ...
            if (parameterRecord == null && _Parameter != null)
            {
        ...
        /// <returns></returns>
        protected void GetListRecordsConcernedByParent(DSCache cache,
                                                       string table,
                                                       DSRecord record,
                                                       string tableParent,
                                                       int id,
                                                       ...
                                                       string area)
        {
            GetListRecordsConcernedByUpdate(cache, tableParent, id, customerId, userId, profile, area, false, null, null);
            cache.Set(table, record.Id, cache.Is(tableParent, id) != true ? null : record);
        }
        ...
        private void ExecuteServiceCredits(JArray lines, string folder)
        {
            ...
            if (File.Exists(creditFile))
            {
                ...
                Regex pattern = new(@"^(?<image>.*) : (?<author>.*) \((?<link>.*)\).*$");
                string rootHttp = ConfigurationManager.ServerHttpRootImages;
                if (rootHttp.Length > 0)
                    rootHttp = rootHttp[0..^1];
                rootHttp = (rootHttp + folder[ConfigurationManager.ServerFileRootImages.Length..]).Replace('\\', '/');
        ...
        private JObject? ExecuteServiceReleaseNotes()
        {
            try
            {
                ...
                {
                    String newLine = expr.Replace(line, "** [<a href=\"" + ConfigurationManager.ServerHttpGitHub + "$1\" target=\"_blank\">$1</a>]", 1);
                    lines.Add(newLine + "<br />");
                }
                                              ...
                                              JObject? identity)
        {
            switch (service)
            {
                ...
                case "Sequence":
                    if (record?["Key"]?.Type == JTokenType.String)
                        return ExecuteServiceSequence(customerId, record["Key"]?.ToObject<string>() ?? String.Empty);

                    return new JObject { ["Value"] = 0 };

                case "SetSequence":
                    if (record?["Key"]?.Type == JTokenType.String)
                        return ExecuteServiceSetSequence(customerId, record["Key"]?.ToObject<string>() ?? String.Empty, record["Value"]?.ToObject<int>() ?? 0);
        ...
        public DatabaseContext() : base(ConfigurationManager.CONNEXION_STRING)
        {
            ...
            if ((Database.Connection as OracleConnection) != null)
                _provider = new Provider.Oracle(Database.Connection, ConfigurationManager.DatabaseSchema);

            if ((Database.Connection as SqlConnection) != null)
                _provider = new Provider.SQLServer(Database.Connection, ConfigurationManager.DatabaseSchema);

            if ((Database.Connection as FbConnection) != null)
                _provider = new Provider.Firebird(Database.Connection, ConfigurationManager.DatabaseSchema);

            if ((Database.Connection as MySqlConnection) != null)
                _provider = new Provider.MySQL(Database.Connection, ConfigurationManager.DatabaseSchema);
        }
    }
}